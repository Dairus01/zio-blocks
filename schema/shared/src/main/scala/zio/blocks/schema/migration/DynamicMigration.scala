              ...
              value match {
                case record: DynamicValue.Record =>
                  if (record.fields.exists(_._1 == fieldName)) {
                    ...
                    Right(DynamicValue.Record(record.fields ++ Chunk.single((fieldName, defaultValue))))
                  }
                case _ =>
        ...
        modifyAtPath(value, parentPath) {
          case record: DynamicValue.Record =>
            if (record.fields.exists(_._1 == fieldName)) {
              ...
              Right(
                DynamicValue.Record(record.fields ++ Chunk.single((fieldName, DynamicValue.Primitive(PrimitiveValue.String(joined)))))
              )
            }
          case other =>
            Left(TypeMismatchError(parentPath, "Record", other.valueType.toString))
  ...
  private def applySplit(
    ...
    selection.values.flatMap(_.headOption) match {
      case Some(DynamicValue.Primitive(PrimitiveValue.String(s))) =>
        ...
        val result: Either[MigrationError, DynamicValue] = Right(value)
            ...
            modifyAtPath(v, parentPath) {
              case record: DynamicValue.Record =>
                if (record.fields.exists(_._1 == fieldName)) {
                  ...
                  Right(
                    DynamicValue.Record(
                      record.fields ++ Chunk.single((fieldName, DynamicValue.Primitive(PrimitiveValue.String(part))))
                    )
                  )
                }
              case other =>
      ...
      case Some(variant: DynamicValue.Variant) =>
        if (variant.caseName.contains(from)) {
          val renamed = variant.caseValue match {
            ...
            case None     => return Left(ValidationError(s"Variant has no value at $at"))
          }
      ...
      case Some(variant: DynamicValue.Variant) =>
        if (variant.caseName.contains(caseName)) {
          ...
          variant.caseValue match {
            ...
            case None => Left(ValidationError(s"Variant has no value at $at"))
          }
        } else {
