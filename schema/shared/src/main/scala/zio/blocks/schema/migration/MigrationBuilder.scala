 ...
 * @param actions
 *   the accumulated migration actions
 * @tparam A
 *   the source type
 * @tparam B
 *   the target type
...
class MigrationBuilder[A, B](
   ...
   *
   * The target selector specifies where the new field should be added. The
   * default value expression provides the initial value.
   *
   * Example: {{` Migration.newBuilder[PersonV0, PersonV1] .addField(_.age,
   * SchemaExpr.Literal(0, Schema.int)) `}}
   *
   * @param target
   *   a selector for the new field location
   * @param default
   ...
   *
   * @param source
   *   a selector for the field to remove
   * @param defaultForReverse
   *   the default value for reverse migration
   * @tparam C
   ...
   *
   * Example: {{` Migration.newBuilder[OldPerson, NewPerson]
   * .renameField(_.fullName, _.name) `}}
   *
   * @param from
   *   a selector for the field to rename
   * @param to
   *   a selector for the new field name
   * @tparam C
   *   the type of the field
   * @return
   *   an updated builder
   ...
   *
   * Example: {{` Migration.newBuilder[Person, Person] .transformField( _.age,
   * _.age, SchemaExpr.Arithmetic( SchemaExpr.Optic(optic(_.age)),
   * SchemaExpr.Literal(1, Schema.int), SchemaExpr.ArithmeticOperator.Add ) )
   * `}}
   *
   * @param from
   *   a selector for the source field
   * @param to
   *   a selector for the target field
   * @param transform
   ...
   *
   * @param source
   *   a selector for the optional field
   * @param target
   *   a selector for the mandatory field
   * @param default
   *   the default value for None cases
   * @tparam C
   *   the base type (without Option)
   * @return
   *   an updated builder
   */
  def mandateField[C](
   ...
   *
   * @param target
   *   a selector for the joined field
   * @param sourcePaths
   *   the selectors for the fields to join
   * @param combiner
   *   the expression that combines the source values
   * @tparam C
   *   the type of the joined field
   * @return
   *   an updated builder
   */
  def joinFields[C](
   ...
   *
   * @param source
   *   a selector for the field to split
   * @param targetPaths
   *   the selectors for the target fields
   * @param splitter
   *   the expression that splits the source value
   * @tparam C
   ...
   * @param caseMigration
   *   a builder function for the case transformation
   * @tparam SumA
   *   the source sum type
   * @tparam CaseA
   *   the source case type
   * @tparam SumB
   *   the target sum type
   * @tparam CaseB
   ...
   *
   * @param at
   *   a selector for the collection field
   * @param transform
   *   the transformation expression for each element
   * @tparam C
   *   the element type
   * @tparam D
   *   the transformed element type
   * @return
   *   an updated builder
   */
   ...
   *
   * @param at
   *   a selector for the map field
   * @param transform
   *   the transformation expression for each key
   * @tparam K
   *   the key type
   * @tparam V
   *   the value type
   ...
   *   a selector for the map field
   * @param transform
   *   the transformation expression for each value
   * @tparam K
   *   the key type
   * @tparam V
   *   the value type
   ...
   *
   * This method validates that the accumulated actions properly transform the
